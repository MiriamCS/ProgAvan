...
class EuclideanDistanceTest {
    ...
    void calculateDistance() {
        ...
        assertEquals(2.8653, knn.distance.calculateDistance(sample, lista), 0.001);   //delta es precisión
        lista.clear();
        //prueba 2
        for (double elem: vect3){lista.add(elem);}
        assertEquals(2.8896, knn.distance.calculateDistance(sample, lista), 0.001);
        lista.clear();
        //prueba 3
        for (double elem: vect4){lista.add(elem);}
        assertEquals(2.7055, knn.distance.calculateDistance(sample, lista), 0.001);
        lista.clear();
        //prueba 4: lista con ceros
        for (double elem: vect5){lista.add(elem);}
        assertEquals(6.5589, knn.distance.calculateDistance(sample, lista), 0.001);
        lista.clear();
    }
}